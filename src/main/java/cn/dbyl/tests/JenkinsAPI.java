...
public class JenkinsAPI {
	...
	public void getTest() {
		...
		RestAssured.given().param("location", "beijing").param("output", "json").param("ak", "sXF8krbnX1LXbz0TpzivvOS2")
				.when().get("/weather").then().body("status", equalTo("success")).log();
	...
	public void getTest1() {
		...
		RestAssured.given().header("Content-Type", "application/json")
				...
				.when().get("/girl").then().statusCode(200).body("girl", equalTo("I'm a girl !")).log();
	...
	public void postTest1() {
		...
		RestAssured.given().header("Content-Type", "application/json")
				.header("User-Agent",
						"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36")
				.when().post("/boy").then().statusCode(200).body("boy", equalTo("I'm a boy !")).log();
